...
// Spec 334 T6 review fix: the "assigned" rule (member OR lead) is exported so the
// /team hub's ยังไม่จัดทีม bubble counts with the SAME rule the roster renders
// with — a lead without a member row must not inflate the tile count.
import { assignedWorkerIdSet } from "@/lib/sa/crew-teams";

describe("assignedWorkerIdSet", () => {
  it("counts a crew lead as assigned even without a member row", () => {
    const set = assignedWorkerIdSet(
      [{ worker_id: "w-member" }],
      [{ lead_worker_id: "w-lead" }, { lead_worker_id: null }],
    );
    expect(set.has("w-member")).toBe(true);
    expect(set.has("w-lead")).toBe(true);
    expect(set.has("w-loose")).toBe(false);
  });
});
