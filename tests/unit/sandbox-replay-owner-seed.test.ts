...
describe("20260813075881 — PRI owner seed must survive an empty-database replay", () => {
  ...
  it("skips the seed with a notice — never an exception — when no users exist either", () => {
    expect(sql).toContain("skipping PRI seed");
  });

  it("the empty-table wedge is gone: no raise over a missing inherit source", () => {
    ...
    expect(sql).not.toMatch(/raise exception[^;]*inherit/);
  });
});
