...
1. ~~**Who authors drawings and polygons?**~~ **RESOLVED 2026-07-27 — the PM tier and above: `PM_ROLES` (`project_manager` · `super_admin` · `project_director`), i.e. `isManagerRole`.** `procurement_manager` owns the on-site TEAMS (spec 330 / #766) but not the drawings. ⚠️ Binding for U2: gate **all three layers** — affordance, server action, and RLS/RPC — per the spec 187 lesson that the quiet middle layer is the one that gets missed and ships affordance-then-refuse. Pin the positive set by iterating the COMPLETE role domain (`Object.keys(USER_ROLE_LABEL)`), never a hand-typed list, as spec 363 U3 did.
