...
- **Forward-only, zero backfill** — `muster_attendance` has **13 real pilot rows** as of 2026-07-24 (ช่างอวย @ TFM โพธิ์ทอง; the spec's earlier "0 rows" was pre-pilot). No data migration needed anyway: all 13 are pre-rework single sessions → the new `session` column defaults them to `regular`, `ot_hours` is already null on every one, and the 13 distinct `(worker_id, work_date)` stay distinct under the composite unique.
...
- `muster_attendance` still `unique (worker_id, work_date)`, **13 pilot rows** (all `ot_hours` null, so the drop-and-recreate of the unique + the `session default 'regular'` are safe); live constraint name confirmed `muster_attendance_worker_id_work_date_key`.
...
export interface MusterMember {
  ...
  inAt: string | null;
  outAt: string | null;
  outAuto: boolean; // the REGULAR session
