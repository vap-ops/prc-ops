...
RPCs: `save_project_zone_map` · `upsert_project_zone` · `delete_project_zone` · `set_wp_zone` · `clone_project_zones(source, target)`. Reads go through RLS on `project_zones` — gated by `can_see_project`, plus a `procurement` / `procurement_manager` bypass arm.

🚨 **CORRECTED 2026-08-06 — the original sentence here claimed "the field roles that already open a WP can see its zone without a new door". That is true for `site_admin` and FALSE for `technician`.** Read live from `pg_get_functiondef`, `can_see_project` has exactly three arms: unconditional true for `{super_admin, project_coordinator, project_director, procurement_manager}`, membership-or-project-lead for `{project_manager, site_admin, site_owner, auditor}`, and **`else false`**. `technician` appears in no arm, so the role never reaches the membership test — a ช่าง is refused **even when they hold a `project_members` row**. Live counts 2026-08-06: `site_admin` 6 users / **5 holding a `project_members` row** ⇒ SA reads through table RLS today; `technician` **14 users / 1 holding one**, and that one is still refused. ⇒ **a zone surface for ช่าง needs a DEFINER RPC, not RLS**, while an SA surface may use table RLS. That is the binding rule in spec 401 §3.1, which carries the same correction. 392's own **U3b**, the ช่าง's zone on `get_my_assigned_work`, is exactly the unit it governs — and per §9 below, re-read the gate live before building it rather than trusting this paragraph.
...
✅ **RESOLVED 2026-08-06 — both the decision and the spec exist. → `docs/feature-specs/401-zone-model-reconciliation.md` (merged, PR #1001).** The operator ruled **`wp_zones` M:N is the single truth and `work_packages.zone_id` is DROPPED** — chosen over keeping it as a derived "primary zone" because a WP like งานพื้นลาน spanning four yards has no non-arbitrary main zone. 392's geometry, editor and four of its five RPCs are reused wholesale; **`set_wp_zone` is retired.** Build order lives in 401 §5 — **U1 junction + `photo_logs.zone_id` → U2 move every reader → U3 drop the column → U4 PM binds → U5 SA sticky capture → U6 gallery-by-zone**, with U7 (the drawings bucket, this spec's deferred U2c) outside that sequence.

⚠️ **Three things still written above are therefore stale, and the still-open unit is the dangerous one:** the **U3b** row in the §7 units table (its two `get_my_assigned_work` columns must come from the junction, not `zone_id`), the fill-rate bullet in §8, and §4's "nullable is correct and **permanent**". 401 §4 carries the reader inventory the drop must budget for; 401 §3 records that a leaf-only `wp_zones` **removes** the **group**-WP zone chip this spec shipped — deliberately, for U4 to rule on, not silently.

🚨 **Do not inherit 401 §4/§5's "two pgTAP assertions" — counted in this file 2026-08-06, the real figure is TEN plus the plan count.** `supabase/tests/database/392-project-zone-maps.test.sql` dies on the U3 drop at: `has_column` for `zone_id`; two `has_column_privilege` pins (SELECT allowed, UPDATE denied); **two `set_wp_zone` membership assertions inside the five-RPC arrays, one of them a `count(*) = 5`**; `has_function_privilege('anon', 'set_wp_zone(uuid, uuid)')`; and four behavioural blocks that call `set_wp_zone` or read `zone_id` back — plus `select plan(37)` itself. ⚠️ **`has_column_privilege` / `has_function_privilege` against a dropped object ERRORS rather than failing**, so the U3 author cannot simply watch these go red. Do not build another zone unit from this file alone — read 401 first.
