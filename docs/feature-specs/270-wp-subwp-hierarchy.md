...
**Status:** DESIGN APPROVED (operator, 2026-07-06) — awaiting FINAL grouping list; U1/U2 buildable now.
...
- **U6 (post-prod-import):** migration adds `CHECK (is_group OR parent_id IS NOT NULL)` and VALIDATEs;
  new-งานย่อย creation UI requires a parent pick from then on.
