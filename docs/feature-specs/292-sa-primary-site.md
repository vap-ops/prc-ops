...
- **`src/lib/sa/current-project.ts`** — PURE: `resolveSaCurrentProject({ visibleProjects, overrideProjectId, queryProjectId? }) → { projectId: string | null; source: "query" | "override" | "primary" | "derived" | "none" }`. _(Amended at U2 build, session 7, per the operator's session brief: the validated `?project=` deep-link is folded INTO the pure resolver as an optional param — source `"query"` — so the whole precedence table lives and is tested in one function. Only the plan page passes it; Precedence #1's plan-page-only, view-only semantics are unchanged.)_ `visibleProjects` = the RLS-scoped project list, each annotated `{ id, code, isPrimary, addedAt, hasMembership }`. A **lead-only** project (visible via `projects.project_lead_id`, no membership row) has `hasMembership: false`, `addedAt: null`, `isPrimary: false` — viewable, not pinnable.
