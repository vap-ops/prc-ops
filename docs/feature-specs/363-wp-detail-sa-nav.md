...
| **U2a** | **SCHEMA.** A SECURITY DEFINER read RPC returning a WP’s status history, gated on `WP_DETAIL_ROLES` + `can_see_project`                                                                                                                                                                                                                                      | **Yes**       | See §4.1 — the SA cannot read `wp_status_transition` rows at all. Precedent: spec 358 U1’s two DEFINER read RPCs, RLS untouched                                                                                         |
| **U2b** | `ประวัติ` tab — one reverse-chronological timeline with day headers and filter chips (`ทั้งหมด · รูป · ตรวจ · ของ · สถานะ`), over the sources the SA can already read. **Retires the `ข้อมูล` tab.**                                                                                                                                                         | No            | Ships without status rows until U2a lands; nothing is lost, because `ข้อมูล` only ever held `ประวัติการตรวจ`, which the timeline carries                                                                                |
...
| **U7**  | `เครื่องมือ` section: tools currently out with aging, `ยืมเครื่องมือ`, `คืน` — activating spec 202 U2. ✅ BUILT 2026-07-28 with spec 370's evidence contract (≥1 condition photo both directions, `via='wp_tab'`, optional ผู้ยืม); loans sort OLDEST first; the borrow door hides on a `complete` WP (mirrors the RPC guard — คืน stays, check_in has none) | ✅ 2026-07-28 | RPC signatures moved under spec 370 U1 the same day: `check_out_equipment(p_item, p_wp, p_date, p_via, p_borrower_worker_id)` · `check_in_equipment(p_log, p_date, p_via)` — legacy call forms still valid via defaults |
...
| source                                           | rows                    | note                                                                                                             |
| ------------------------------------------------ | ----------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `audit_log` where `target_table='work_packages'` | 559 / 30 d              | ⚠️ **NOT SA-READABLE — see §4.1.** `action` is `'other'` on every row, so any reader must key on `payload.event` |
...
### 4.1 ⚠️ The site admin cannot read status transitions (found 2026-07-27, before any U2 code)

`audit_log` carries two SELECT policies. The privileged one admits `super_admin`, `project_director`, `accounting`, `project_manager` and returns everything. The other admits `site_admin`, `procurement`, `procurement_manager` — and restricts them to:

```sql
payload->>'event' = ANY (ARRAY['wp_reopened_for_defect', 'wp_evidence_resubmitted'])
```

**`wp_status_transition` is not in that allowlist.** A timeline built naively over `audit_log` would therefore render complete for a PM and silently omit every status change for the site admin — the role the tab exists for. This is a role-differential hole that no PM-side test would catch, which is exactly why the check happened before the code and not after.

Two consequences, both folded into §3's unit table:

1. **U2b (code) ships over the sources the SA can already read** — verified live the same day: `approvals` (policy `approvals readable by sa/pm/super`), `stock_issues` (`can_see_project`), `purchase_requests` (own-or-privileged), `photo_logs`, plus the two rework events above. That is a real timeline, and critically it carries `ประวัติการตรวจ`, so retiring the `ข้อมูล` tab loses nothing.
2. **U2a (schema) adds the status rows** via a SECURITY DEFINER read RPC gated on `WP_DETAIL_ROLES` + `can_see_project`, following spec 358 U1's pattern (definer read RPCs with their own gate; RLS left alone).

**Why an RPC rather than widening the policy:** the SA arm has **no project scoping at all** — it is role + event only. Adding `wp_status_transition` to it would expose the status history of every project in the system to every site admin, not just the ones they are on. The RPC scopes to the WP and can be pinned by pgTAP. Widening the allowlist is the cheaper diff and the wrong one.
...
4. **Bulk vs unit equipment.** ✅ ANSWERED (operator, 2026-07-28 — spec 370 D5): units only in v1; bulk rows are excluded from ยืมเครื่องมือ (filtered from the picker AND refused in the server action, since the RPC has no tracking guard) and move only as site-level movements.
