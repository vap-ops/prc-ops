...
| U      | Ships                                                                                                                                                     | DB? | Notes                                                            |
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ---------------------------------------------------------------- |
| **U1** | ⓘ sheet absorbs `หมายเหตุ`; the button renders unconditionally; `ข้อมูล` tab deleted; `ประวัติการตรวจ` moves to U2's timeline                             | No  | Touches `work-package-info-button.tsx`, the page's tab array     |
...
| **U7** | `เครื่องมือ` section: tools currently out with aging, `ยืมเครื่องมือ`, `คืน` — activating spec 202 U2                                                     | No  | `check_out_equipment` / `check_in_equipment` already live        |

**Suggested order: U1 → U3 → U4 → U5 → U2 → U6 → U7.** Cheap structure first; the 93%-traffic camera path (U5) touched only once the merge is field-proven; the only genuine new build (U7) last.
