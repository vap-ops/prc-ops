...
- **Bands map onto existing entities, no new storage:**
  ...
  - ทีมภายนอก = `contractors` (+ `workers.contractor_id`) — today 10 rows (2 marked "⚠ ทดสอบระบบ … (ไม่ใช้งาน)"); only **ช่างอวย has any named workers (3)** — the other 8 active firms have zero.
...
- **`/workers`' `create_worker`/roster-add flow already accepts `contractor_id`** — the door for "an existing firm with zero named workers" to add a person by hand goes there.
- **`/team`'s existing per-firm QR onboarding poster (spec 328)** is the self-onboard door for that same "existing firm, zero names" case.
...
2. **ทีมภายนอก · ผู้รับเหมาช่วง** · count = active contractors. Card = firm identity (spec 338, kept) + member chips when named. A firm with zero named workers renders a dashed card with two doors: `แชร์ QR สมัคร` (existing per-firm poster) and `เพิ่มในระบบเอง` (existing `/workers` add-worker, `contractor_id` preset). Band header gets a new **`+ เพิ่มผู้รับเหมาช่วง`** action → doors to `/contacts`'s existing create-firm flow (new for this page; not a new flow — a door to one).
...
1. **Unassigned workers**: active workers in no crew and tied to no contractor → `"N คนยังไม่มีทีม"` → tap opens the existing add-to-crew sheet pre-scrolled to the unassigned pool.
2. **Leadless crew**: any active crew with `lead_worker_id is null` → `"ทีม X ยังไม่มีหัวหน้าทีม"` → tap opens that crew's card, focused on the existing "ยังไม่ตั้งหัวหน้าทีม" prompt.
3. **Firm with zero named workers**: any active contractor with zero workers under it → `"บริษัท X ยังไม่มีรายชื่อ"` → tap opens that firm's card (the two-door state above).
...
- Real-flow: dev-preview login → `/projects/<pilot>/team` — confirm the fix-list shows the real 8-unassigned + real zero-worker-firm items (NOT a fabricated leadless-crew item, since none exists live), the plan tab carries its count, `+ เพิ่มผู้รับเหมาช่วง` reaches `/contacts`. Zero console errors.
