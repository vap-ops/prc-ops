...
3. **Firm with zero named workers**: a contractor with ≥1 `workers` row (active or inactive) scoped to this `project_id`, but zero currently-active → `"บริษัท X ยังไม่มีรายชื่อ"` → tap opens that firm's card (the two-door state above). **Live today this fires for none of the 10 company contractors** — same as the leadless-crew condition, built for when it happens rather than illustrated as happening now.
