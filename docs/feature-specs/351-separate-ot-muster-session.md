...
- Muster is live but **unused**: `muster_attendance` = **0 rows all-time** (1 test team, 1 closure; live-verified 2026-07-24). So this rework is **forward-only — zero backfill** — and reshaping the schema now, before real data accrues, is the cheap moment (retrofitting a session split onto populated attendance would be far worse).
