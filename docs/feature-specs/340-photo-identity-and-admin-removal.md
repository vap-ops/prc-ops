...
| D6  | **The number renders on the two capture grids** — the WP-detail zone strip and the capture sheet. Those are what gets screenshotted. **Deferred:** the lightbox (where the delete is confirmed) and `phase-gallery` (the reviewer-facing `/review` surfaces) still show no number; threading `seq` through `ZoomablePhoto` → the overlay, and into the gallery, is its own unit. Until then the PM cannot quote a number when asking for a re-shoot — the gap is recorded, not hidden. |
| D7  | **super_admin may not delete on their OWN ให้แก้ไข decision** (fresh-eyes catch). `super_admin` is in the `approvals` INSERT policy, so the operator can be the decider — without this they could bounce a WP and then delete the uploader's photo, which is precisely the approver-alters-the-evidence hazard 291 closed, reopened for one principal. `is distinct from` so a null `decided_by` cannot open it either. Same shape as spec 337 U2, where the decider may not answer their own bounce. |
| D8  | **"Delete-on-behalf could become substitute-on-behalf" is refuted by the table, not by RLS.** The review flagged that the INSERT policy admits any row carrying `superseded_by`, including one with a `storage_path` (a replacement image). It cannot: `check ((storage_path is null) = (superseded_by is not null))` makes every superseding row a tombstone, for every role, before and after this spec. Migration `075834` added a policy conjunct for it and `075835` removes that conjunct again — an unreachable clause in a security policy implies a live hazard and costs a per-row definer call on a path that never runs. The pgTAP now pins the CONSTRAINT (23514), which is the layer that actually holds. |

## Unit U1 — super_admin delete-on-behalf (schema, migrations `075833`–`075835`)
