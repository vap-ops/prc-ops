...
U10 = the count UI — per-row `ตรวจนับ` button on /store (manager-tier) → counted-qty sheet with a live variance preview → `recordStockCount`. Code-only. NEXT = reversals, then the late margin layer.
