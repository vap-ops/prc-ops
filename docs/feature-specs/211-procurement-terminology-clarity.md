...
**Cross-engine reconcile _(SHIPPED — own unit)_:** `terminology-05`/`worklist-hub-16`, critic gap X2. The SITE band engine (`request-bands.ts`, for site_admin/PM/super) and the procurement pipeline (`procurement-pipeline.ts`, for procurement) are two band engines over one table. **Finding:** a user only ever sees ONE engine (procurement → pipeline; site roles → request-bands; gated by `isProcurement` in `requests/page.tsx`), so there is **no same-screen contradiction** (that was U7). The real issue was **drift** — the terms the two engines genuinely share (`รออนุมัติ`, `กำลังจัดส่ง`, the `รอสั่งซื้อ` core) were duplicated literals. **Fix:** new `src/lib/purchasing/worklist-band-vocab.ts` `WORKLIST_BAND_TERM` SSOT; both engines source the shared terms from it (procurement verbatim; site's `to_order` = `อนุมัติแล้ว ` + the shared core). The **deliberate per-view differences are PRESERVED** (site terminal `done`=`เสร็จแล้ว` / `closed` vs procurement `received`=`ได้รับแล้ว` — each audience-correct) and documented, NOT force-unified. Behaviour-preserving (every rendered string byte-identical). Test-first `worklist-band-vocab.test.ts` (shared terms derive from the SSOT + exact-string pins + a guard that the per-view terminal framing stays distinct). Code-only.

### U8 — Notification PO-awareness _(SHIPPED — held danger path)_

Critic gap X1. `compose-notification.ts` pushed raw PR status words to LINE/Telegram with no PO context — the headline confusion reached the user pre-screen. **Fix (code-only, no migration):** a PR notification now names its parent ใบสั่งซื้อ when it has one — `คำขอซื้อ PR-0012 · ใบสั่งซื้อ PO-0003: กำลังจัดส่ง`. The PO is resolved at **compose time** in the drain route (`src/app/api/notifications/drain/route.ts`): the outbox row already carries `purchase_request_id`, so a batched 2-hop lookup (PR → `purchase_order_id` → `po_number`) builds a `poNumberByPrId` map, passed into the new `ComposeContext.poNumber` — mirroring the existing `wpCode` enrichment, NO payload/trigger change. The composer also drops its **duplicated** local `formatPrNumber` for the U2 SSOT (`formatPoNumber`/`formatPrNumber`), via a new `prRef()` helper used by all four PR cases (the U2 note's deferred SSOT adoption). Absent PO → message unchanged (the existing tests stay green). Test-first `compose-notification.test.ts`. `src/lib/notifications/**` + `src/app/api/notifications/**` are danger paths → held for operator merge.
...
**U9b — voucher → PO live link _(SHIPPED; operator chose Option A 2026-06-27)_:** the voucher's PO becomes a live link into the PO detail. The blocker (the voucher is `ACCOUNTING_ROLES`-gated but the PO detail was `PURCHASING_ROLES`-gated, excluding `accounting`) is resolved by **opening the PO detail to accounting, read-only** — new `PO_DETAIL_VIEW_ROLES = PURCHASING_ROLES + accounting` gates the route; accounting reads the PO via the **admin client** (its org-wide money posture, like the voucher/register — no RLS grant/migration needed), sees the money (`canSeeMoney = isBackOffice || isAccounting`), but the write actions (`canManageDeliveries`/`canReceive`) stay gated out (they key on `isBackOffice`/`RECEIVE_ROLES`, neither admits accounting). `load-voucher` now exposes `poId`; the voucher's `ใบสั่งซื้อ` META row links to `/requests/orders/[poId]` with `?from=` so its back chip returns to the voucher (the PO detail back chip gains `?from` via `safeBackHref`). Test-first `role-sets.test.ts` (`PO_DETAIL_VIEW_ROLES`). **Auth/access change → operator-held PR.**

**U9c — register grouped by PO _(SHIPPED)_:** `accounting-ap-03`. The accounting purchase register now reads a PO at a time — each PO group has a `<PoNumberTag>` header + a gross subtotal, with the direct/site buys (no PO) grouped last (`ซื้อตรง (ไม่มีใบสั่งซื้อ)`). New pure `groupRegisterByPo()` SSOT in `purchases-view.ts` (generic, satang-summed, PO groups in first-appearance order, no-PO last); `load-purchases` resolves a `poNumber` per row (joins `purchase_orders`). Test-first `purchases-view.test.ts`. Display-only, no schema; `src/app/accounting/**` → held.
...
- **U11c — unified self-purchase _(operator-approved 2026-06-27: bounded design)_:** one guided `ซื้อเอง` form replacing U11a's two cards. **Posting-model finding** (subagent map of the current model): of the 3 axes — item {catalog/free-text} × VAT {invoice/cash} × flow {use-now/record-only} — 5 of 8 combinations work today; the only REAL missing one is **catalogued + VAT + use-now**. Free-text + use-now is structurally impossible (the store/stock tables key on `catalog_item_id`) AND not a real gap — `record_site_purchase` already books a free-text item's cost straight to the WP (`Dr 1400 / Cr 2100`), i.e. already "bought and used". So the bounded plan, NOT a "store holds free-text" redesign:
  - **U11c-A — VAT on use-now _(money migration, operator-held)_:** add `p_vat_rate` to `site_purchase_use_now`; store the receipt at NET + snapshot `vat_rate`, so the receipt poster (`post_stock_receipt_to_gl`) splits `Dr 1300 Input VAT` — its no-PR fallback (`v_gross := round(v_net*(1+rate/100),2)`, spec 208 U4b) already supports exactly this. Mirrors `purchase_requests_stock_in_on_receive`'s net pattern. Zero-VAT byte-identical. DROP+CREATE (new 7-arg signature); pgTAP 228 gains a VAT-split case + signature pins updated.
  - **U11c-B — the unified form _(code-only)_:** one `ซื้อเอง` form: item (catalog search OR free-text), qty, amount, `มีใบกำกับภาษี?`, and — catalog item only — `ใช้ที่งานนี้เลย?`. Routes: catalog + use-now → `site_purchase_use_now` (now VAT-aware); else → `record_site_purchase`. Free-text doesn't offer use-now (non-catalogued = not stock-trackable). Makes 7/8 combos reachable; the 2 free-text-use-now cells stay closed on purpose.
