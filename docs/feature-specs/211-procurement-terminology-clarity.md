...
- **U11b — item-image upload _(next)_:** add an item-photo uploader to self-purchase (existing `reference` purpose, an `addReferenceAttachment` action mirroring `addInvoiceAttachment`), alongside the docs uploader `#2` already has — so a self-purchase carries both the item image and the receipt/invoice. Code-only (no enum/migration).
