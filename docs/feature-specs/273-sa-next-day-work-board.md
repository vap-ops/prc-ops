...
| **U5** | code   | **Date-navigable board (editable, not tomorrow-only).** `/sa/plan?date=` param → `resolvePlanDate` (default พรุ่งนี้; floor at today; reject malformed/past). Board gains a ◀/▶ day stepper (prev disabled at the today floor) + วันนี้/พรุ่งนี้ qualifier; project switch preserves the date. `/sa` gains an "แก้ไขแผนวันนี้" deep-link when a today board exists. **Reuses the U1 RPCs unchanged** — `add_daily_plan_item` already takes `p_date`, so this is code-only. vitest first. **✅ SHIPPED.** | U2         |
