...
// section label, list rows — on the app's zinc-950 ground so the swap to
// real content doesn't flash. Explicit zinc tones override the shadcn
// Skeleton's theme-token default (the screens hardcode the dark palette).
    ...
    <main className="min-h-screen bg-zinc-950">
      ...
      <header className="border-b border-zinc-800 px-5 py-4">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          ...
          <Skeleton className="h-8 w-28 bg-zinc-800/80" />
        </div>
      </header>
      <div className="mx-auto max-w-3xl px-5 py-6">
        <Skeleton className="mb-4 h-4 w-28 bg-zinc-800/80" />
        <div className="flex flex-col gap-2">
          {[0, 1, 2, 3].map((i) => (
            <Skeleton key={i} className="h-16 w-full rounded-lg bg-zinc-900/80" />
          ))}
        </div>
      </div>
    </main>
