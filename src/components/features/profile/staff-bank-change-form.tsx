...
// Spec 317 U2/U4 — an approved office staffer stages a bank change (APPROVED
// tier; the staff-approval trio decides; approve upserts staff_registration_bank
// + chains the passbook into the registration docs). Passbook photo REQUIRED —
// uploads to the caller's own technician/<uid>/book_bank/ folder at submit,
// then the action passes {attachmentId, ext} for the server to rebuild the
// path. Mirrors the worker bank-change form.
// 'use client': form + upload state + the server-action call.

import { useRef, useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { submitStaffBankChange } from "@/app/settings/my-info/actions";
...
export function StaffBankChangeForm({ uid, hasPending }: { uid: string; hasPending: boolean }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      ...
      const result = await submitStaffBankChange({
        bankName,
        accountNo,
        accountName,
        attachmentId,
