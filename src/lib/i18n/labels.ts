...
// ---------------------------------------------------------------------------
// Spec 271 U2a — plan-vs-actual vocabulary (ADR 0075). The class labels mirror
// the DB `variance_class` enum 1:1 (compile-pinned via the Record type below +
// runtime-pinned in tests/unit/wp-variance.test.ts). Thai set sent to the
// operator for confirmation before this unit merges (spec §6/§10 gate); the
// U4/U5 surface terms ship here now so every later unit reads ONE source.
import type { Database as LabelsDb } from "@/lib/db/database.types";

export const VARIANCE_CLASS_LABEL: Record<LabelsDb["public"]["Enums"]["variance_class"], string> = {
  on_track: "ตามแผน",
  at_risk: "ใกล้ครบกำหนด",
  late: "ช้ากว่าแผน",
  late_start: "เลยกำหนดเริ่ม",
  never_started_past_end: "ไม่ได้เริ่ม เลยกำหนดจบ",
  completed: "เสร็จ",
  completed_undated: "เสร็จ (ไม่ทราบวันที่)",
  no_evidence: "ยังไม่มีข้อมูล",
  unplanned: "ไม่มีแผน",
};

/** Baseline lens toggle (U2b schedule surface). */
export const BASELINE_LENS_LABEL = "แผนอ้างอิง";
/** Auditor's per-งาน acceptance tap (U5). */
export const SIGNOFF_LABEL = "ตรวจรับงาน";
/** Client punch-list intake (U4). */
export const CLIENT_DEFECT_LIST_LABEL = "รายการแก้ไขจากลูกค้า";
/** site_owner recorded directive (U5). */
export const SITE_DIRECTIVE_LABEL = "คำสั่งหน้างาน";
/** Coverage caption on a suppressed (grey) variance pill. */
export function varianceCoverageCaption(pct: number): string {
  return `หลักฐาน ${pct}%`;
}
