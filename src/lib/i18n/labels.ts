...
/** Spec 343 U3 — "bring your ID card before you scan", led into the LINE-share
 *  text AND printed on the poster (same constant, both surfaces), so a firm
 *  member knows what to prepare before the QR is even open. */
export const REGISTER_PREP_POSTER_LINE = "เตรียมบัตรประชาชนก่อนสแกน";
...
// Spec 395 U4 — working the flagged accounts, ON THE ROSTER.
//
...
// on it reads "probably a typo in the name or the number" — §5's `044…`/`014…` near-miss
// being the likely example. Without this the reviewer cannot tell the two apart.
export const PAYOUT_ACCOUNT_SHARED_WITH_PREFIX = "ช่างคนอื่นที่ใช้บัญชีนี้:";
...
/** "Waited N days" — the SSOT for every queue-aging chip. Spec 371 adopted the
 *  wording already shipped on the PO worklist rather than minting a second
 *  phrasing for the same idea (memory `ui-term-consistency-ssot`). */
export function waitingDaysChip(days: number): string {
