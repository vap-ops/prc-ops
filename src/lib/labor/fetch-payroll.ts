...
// requireRole(PM_ROLES) before invoking. Fetches ALL worker types in the window
// — DC is filtered in aggregatePayroll AFTER the current-state pass (a supersede
// correction re-snapshots worker_type; a DB-level type filter could miscount).
...
export async function fetchPayrollReport(
  ...
  const { data: rows, error } = await admin
    .from("labor_logs")
    .select(
      "id, worker_id, worker_name_snapshot, worker_type_snapshot, day_fraction, day_rate_snapshot, superseded_by, work_date",
    )
