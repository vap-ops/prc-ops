...
// Spec 319 — decide a login-keyed (admin/office) bank change. Trio only, exactly
// like the staff-bank and identity deciders; approve upserts user_bank (the RPC
// re-gates + late-bind rechecks worker/contractor/approved-reg in one txn).
export async function decideUserBankChange(input: {
  id: string;
  approve: boolean;
  revalidate: string;
}): Promise<ActionResult> {
  ...
  const { error } = await auth.supabase.rpc("decide_user_bank_change", {
    p_id: input.id,
    p_approve: input.approve,
  });
  if (error) return { ok: false, error: GENERIC_BANK };
