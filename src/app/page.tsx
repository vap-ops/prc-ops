...
export default async function Home() {
  ...
  if (user) {
    const { data: row } = await supabase
      ...
      .maybeSingle();
    if (row) {
      // Land by role. Operator 2026-07-06: a single-project site_admin used to land
      // on their project — reverted so /sa (the SA daily home, where the แผนพรุ่งนี้
      // board lives) is always the landing.
