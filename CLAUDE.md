...
- Skills at `.claude/skills/` provide procedural knowledge. Currently installed: `ship-unit` (the gated unit loop — load for EVERY unit); `supersede-pattern`; `bug-fix-flow` (the autonomous bug-fix pipeline — discover→triage→fix→ship→reply→complete, driven by CC, flagging the operator only at genuine decision points; runs scheduled daily + on demand); `triage-feedback` (spec 201 — the queue/message/status mechanics `bug-fix-flow` builds on: CC investigates reports, sets status off `ใหม่`, and **always stages a draft for the operator to publish — spec 405 (2026-08-08) RETIRED the auto-publish tier, and CC never writes to `feedback_messages` directly**). Load them when touching matching areas.
- Hooks at `.claude/hooks/` enforce constraints automatically. Currently installed: `protect-audit-log.js` (blocks edits to audit_log migrations unless `CLAUDE_ALLOW_AUDIT_LOG_EDIT=1` is set); `require-lane-claim.js` (blocks migration writes unless the current branch is claimed in `../LANES.md`; always blocks migrations on `main`); `bash-guards.js` (Bash matcher, three checks in one script — ① a repo-tool command (`pnpm`/`npx`/`vitest`/`supabase`/`tsx`, or `bash|sh|node scripts/…`/`./scripts/…`) must START with an absolute `cd` — the cwd-drift wall; ② blocks the common shell-write shapes (`sed -i`, redirects INTO the tree, `rm`/`mv`/`cp`, `git checkout --`) touching `supabase/migrations|tests`, which would bypass the two file-tool guards above — heuristic belt, not a vault; ③ blocks Bash reads of `.telegram.env`/`.github.env` (creds must not enter the transcript — the pwsh send flows load them in-process; heuristic belt: verb-list based, `node -e`/glob/quoted-path readers slip). Escape per check: append the in-command marker `#ALLOW_NO_CD <why>` / `#ALLOW_BASH_SCHEMA_WRITE <why>` / `#ALLOW_SECRET_READ <why>`, or launch the session with the matching env var). Do not attempt to bypass silently — use the markers, they leave an audit trail.
- Subagents at `.claude/agents/` package recurring specialized reviews. Currently installed: `spec-fact-checker` (refute-first fact-checker — verifies the factual claims in a spec, plan, or task-prompt against the live DB and the code at your worktree HEAD, returning a claim-by-claim verdict table; the doctrine §4 gate); `unit-reviewer` (the ship-unit Gate-4 fresh-eyes reviewer — opus-pinned so the last adversarial gate before a ship never silently runs on a small model; carries the repo's failure-class lenses). Dispatch spec-fact-checker BEFORE shipping any spec/plan and before building from a task-prompt or audit finding that names tables, columns, or files; dispatch unit-reviewer on every unit's full diff at Gate 4.
