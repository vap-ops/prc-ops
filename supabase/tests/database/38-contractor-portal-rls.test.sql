...
-- internal role-level access (site_admin) is unchanged; the wage_payments money
-- posture (zero authenticated grant) is preserved — a DC reads their own money
-- through get_my_wage_payments(), not the raw table.
--
-- Spec 170 U3 (ADR 0062): wage_payments is now keyed on worker_id. The portal
-- binding is still via the contractor until U4, so get_my_wage_payments() bridges
...
-- Spec 170 U4a: the portal now binds on workers.user_id (get_my_wage_payments is
...
insert into public.workers (id, name, pay_type, employment_type, contractor_id, user_id, day_rate, active, created_by) values
  ('a2000000-0000-4000-8000-000000000138', 'Worker A', 'daily', 'permanent', 'aa000000-0000-4000-8000-000000000138', 'a1000000-0000-4000-8000-000000000138', 400.00, true, '51000000-0000-4000-8000-000000000138'),
  ('b2000000-0000-4000-8000-000000000138', 'Worker B', 'daily', 'permanent', 'bb000000-0000-4000-8000-000000000138', 'b1000000-0000-4000-8000-000000000138', 450.00, true, '51000000-0000-4000-8000-000000000138');
...
insert into public.labor_logs (id, work_package_id, worker_id, work_date, day_fraction,
  ...
  ('fa000000-0000-4000-8000-000000000138', 'e0000000-0000-4000-8000-000000000138', 'a2000000-0000-4000-8000-000000000138',
   date '2026-06-05', 'full', 400.00, 'Worker A', 'daily', '51000000-0000-4000-8000-000000000138'),
  ('fb000000-0000-4000-8000-000000000138', 'e0000000-0000-4000-8000-000000000138', 'b2000000-0000-4000-8000-000000000138',
   date '2026-06-05', 'full', 450.00, 'Worker B', 'daily', '51000000-0000-4000-8000-000000000138');
...
select is((select count(*) from public.labor_logs),
  1::bigint, 'uA sees only their own labor day (self-worker read; bound-contractor labor_logs read retired in the DC→ch merge, reworked in U7)');
select throws_ok(
...
select throws_ok(
  ...
  '42501', null, 'wage_payments stays zero-grant — uA cannot read it directly');
...
select is((select paid_amount from public.get_my_wage_payments() limit 1),
...
select is((select count(*) from public.labor_logs
            where worker_id = 'a2000000-0000-4000-8000-000000000138'),
  0::bigint, 'uB cannot see Worker A''s labor days (self-worker isolation)');
select is((select worker_id from public.get_my_wage_payments() limit 1),
