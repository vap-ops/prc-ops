-- Spec 270 U6 follow-up — import_wp_grouping: phase C creates new งานย่อย WITH
-- their parent set at INSERT time. 072500 made a parentless งานย่อย INSERT illegal
-- in any project that already has งาน rows — which is every project mid-re-import
-- (its own phase B just created them). The old phase C inserted parentless and
-- wired parents in phase D, so any re-import on an adopted project would 23514.
-- Same body as 072400 otherwise.
...
begin
  ...
  with new_leaves as (
    ...
    select p_project_id, '~new~' || t.ord, t.name, g.wp_id
    from _imp_ids t
    left join _imp_ids g on g.code = t.sub_of and g.is_group_row
    where not t.is_group_row and t.existing_id is null
    returning id, code
  )
  update _imp_ids t
    set wp_id = nl.id
