-- Spec 134 U7 / ADR 0053 — delivery batches. A partial delivery forks the PO into
-- batches (งวดส่ง): lines received together form one batch; the rest stay pending.
-- `delivery_batch_id` stamps the lines received in a single receive event so the PO
-- detail can show the delivery breakdown (and U4b/Lalamove can map a courier order
-- onto a batch). RPC-only by construction — the column-scoped authenticated UPDATE
-- grant (20260616000400) does not name it, so only the SECURITY DEFINER receive/
-- split RPCs (function owner) write it (ADR 0038 fact-column posture).

alter table public.purchase_requests
  add column delivery_batch_id uuid null;

create index purchase_requests_delivery_batch_id_idx
  on public.purchase_requests (delivery_batch_id);

-- receive_po_lines: one receive call = one batch. Stamp every received line with a
-- freshly minted batch id (the rest of the body is unchanged from 20260715000000).
...
-- split_purchase_request_on_receipt: the delivered portion is its own delivery batch
-- (a batch of one — the partial that arrived). The remaining child stays on_route
-- with no batch. Body unchanged from 20260714000000 except the delivery_batch_id
-- stamp on the original's UPDATE.
